...
class Brain(BaseBrain):
  def __init__(self,Robot):
    ...
    Body.addToMemory('ground_direction', None)
    Body.addToMemory('ground_tendencies', [None, None, None])
    Body.addToMemory('new_beam_steps', 0)

    # Same as above, but on the ground - since the ground is already a horizontal
    # support beam
    Body.addToMemory('new_beam_ground_steps', 0)

    # Stores name of beam to be reinforced (so we know when we're no longer on it)
    Body.addToMemory('broken_beam_name','')

    # Stores the previous beam we were on
    Body.addToMemory('previous_beam',None)

    # Smaller number gives higher priority
    Body.addToMemory('dir_priority',[0,0,0])

    # Move further in the x-direction?
    Body.addToMemory('pos_x', None)

    # Move further in the y-direction?
    Body.addToMemory('pos_y', None)

    Body.addToMemory('start_construction', False)

    # Climbing up or down
    Body.addToMemory('pos_z', None)
    Body.addToMemory('dir_priority', [0])
    ...
    Body.addToMemory('previous_direction', None)

    # Stores information on beams that need repairing
    Body.addToMemory('broken', [])
    ...
    Body.addToMemory('preferred_direction', None)
    ...
    Body.addToMemory('ground_direction', helpers.make_vector(self.Body.getLocation(),
      CONSTRUCTION['corner']))

    # The direction in which we should move
    Body.addToMemory('next_direction_info', None)
